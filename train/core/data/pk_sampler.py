...
def _create_groups(targets: List[int], k: int) -> DefaultDict[int, List[int]]:
    """
    ...
    for class_ in classes_to_extend:
        ...
        n_samples_to_extend = k - n_samples
...
class PKSampler(Sampler):
    ...
    Args:
        ...
        labels_per_batch (int): Number of labels/classes to be sampled in a batch
        samples_per_label (int): Number of samples per label/class in a batch
    """
    ...
    def __init__(
        ...
        self.labels_per_batch: int = labels_per_batch
        self.samples_per_label: int = samples_per_label
        self.class_to_idxs: DefaultDict[int, List[int]] = _create_groups(
            targets, self.samples_per_label
        )

        self.sequential_sampling: bool = sequential_sampling

        # Ensures there are enough classes to sample from
        assert len(self.class_to_idxs) >= labels_per_batch
    ...
    def __iter__(self) -> int:
        ...
        if not self.sequential_sampling:
            ...
            while len(class_to_n_samples) >= self.labels_per_batch:
                ...
                selected_class_idxs: List[int] = torch.multinomial(
                    torch.ones(len(classes)), self.labels_per_batch
                ).tolist()

                for i in selected_class_idxs:
                    class_: int = classes[i]
                    # List of samples' indexes in a particular class
                    ...
                    if class_to_n_samples[class_] < self.samples_per_label:
